...
def plot_model_panel(
    ...
    95% CIs are shown as shaded areas.
    """
    ...
    # ------------------------------------------------------------
    # Main selection-rate curves with shaded 95% CI
    # ------------------------------------------------------------
    for attribute_value in attribute_order:
        ...
        xs = np.array(sorted(res.keys()), dtype=float)
        xticks = list(xs)

        ys = np.array([res[x]["hit_rate"] for x in xs], dtype=float)
        ci_low = np.array([res[x]["ci_low"] for x in xs], dtype=float)
        ci_high = np.array([res[x]["ci_high"] for x in xs], dtype=float)
    ...
    if len(xticks) > 0:
        ...
        ax_main.set_xlim(min(xticks) - 0.12, max(xticks) + 0.12)
    ...
    # ------------------------------------------------------------
    # Delta curve on right y-axis with shaded 95% CI
    # ------------------------------------------------------------
    if "delta" in all_results:
        ...
        xs_delta = np.array(sorted(delta_res.keys()), dtype=float)
        ys_delta = np.array(
            [delta_res[c]["delta"] for c in xs_delta],
            dtype=float,
        )
        ci_low_delta = np.array(
            [delta_res[c]["ci_low"] for c in xs_delta],
            dtype=float,
        )
        ci_high_delta = np.array(
            [delta_res[c]["ci_high"] for c in xs_delta],
            dtype=float,
        )

        # Shaded 95% CI for delta
...
def draw_attribute_big_figure(
    ...
    base = f"{safe_slug(attribute_type)}_all_applications_contextual_nature_style_shaded_area"
