...
def plot_model_panel(ax_main, attribute_type, resume_count, all_results, significance, model_name):
    ...
    if model_name == "GLM-4.5-Air" and attribute_type == "Race":
...
def draw_results_grid(attribute_type, resume_count, model_names, pool_count, max_n_trials):
    ...
    for idx, model_name in enumerate(model_names):
        ...
        if "no_thinking" in model_name:
            file_name = f"outputs/loan/contextual/{attribute_type}/{model_name[:-12]}_{resume_count}_{pool_count}_no_thinking.jsonl"
        else:
    ...
    # Save one big figure
    save_file = f"outputs/loan/contextual_grid_{attribute_type}_{resume_count}_no_thinking.png"
