...
def draw_results(model_name, attribute_type, resume_count, all_results, significance):
    ...
    # -----------------------------------------------------------
    # Create simple figure and main axis (no bracket panel)
    # -----------------------------------------------------------
    fig, ax_main = plt.subplots(dpi=1024)

    model_name_clean = model_name.replace("msra-", "")
    fig.suptitle(
        f"Loan - {attribute_type} ({model_name_clean})",
        fontweight="bold",
        y=0.93
    )
    ...
    save_file = f"outputs/loan/contextual_{model_name_clean}_{attribute_type}_{resume_count}.png"
    ...
    for attribute_type in ["Gender", "Race"]:
        for resume_count in [5, 10]:
            for model_name in [
                ...
                file_name = f"outputs/loan/contextual/{attribute_type}/{model_name}_{resume_count}_{pool_count}.jsonl"
