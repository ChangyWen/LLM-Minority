...
def draw_results_grid(attribute_type, resume_count, model_names, pool_count, max_n_trials):
    ...
    fig.suptitle(f"EDU - {attribute_type}", fontweight="bold", y=0.97)
    ...
    for idx, model_name in enumerate(model_names):
        ...
        if "no_thinking" in model_name:
            file_name = f"outputs/edu/contextual/{attribute_type}/{model_name[:-12]}_{resume_count}_{pool_count}_no_thinking.jsonl"
    ...
    # Save one big figure
    save_file = f"outputs/edu/contextual_grid_{attribute_type}_{resume_count}_no_thinking.png"
