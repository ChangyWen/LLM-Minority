...
def draw_results(model_name, attribute_type, resume_count, all_results, significance):
    ...
    fig.suptitle(
        f"Promotion - {attribute_type} ({model_name_clean})",
        fontweight="bold",
        y=0.93
    )
    ...
    save_file = f"outputs/promotion/contextual_{model_name_clean}_{attribute_type}_{resume_count}.png"
    ...
    for attribute_type in ["Gender", "Race"]:
        for resume_count in [5]:
            for model_name in [
                ...
                file_name = f"outputs/promotion/contextual/{attribute_type}/{model_name}_{resume_count}_{pool_count}.jsonl"
